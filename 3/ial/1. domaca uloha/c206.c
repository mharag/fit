...
void DLL_DeleteFirst( DLList *list ) {
    if(list->firstElement){
        DLLElementPtr new_start = list->firstElement->nextElement;
        new_start->previousElement = NULL;
        if(list->activeElement == list->firstElement)
            list->activeElement = NULL;
        free(list->firstElement);
        list->firstElement = new_start;
        if(!list->firstElement)
            list->lastElement = NULL;
    }
}
...
void DLL_DeleteLast( DLList *list ) {
    if(list->lastElement){
        DLLElementPtr new_end = list->lastElement->previousElement;
        new_end->nextElement = NULL;
        if(list->activeElement == list->lastElement)
            list->activeElement = NULL;
        free(list->lastElement);
        list->lastElement = new_end;
        if(!list->lastElement)
            list->firstElement = NULL;
    }
}
